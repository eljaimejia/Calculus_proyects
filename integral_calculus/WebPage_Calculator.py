...
with st.sidebar:
    # Logo oficial de la Universidad Militar Nueva Granada
    st.image("https://upload.wikimedia.org/wikipedia/commons/e/ec/Escudo_Universidad_Militar_Nueva_Granada.svg", width=120)
...
st.write("Escribe tu función, ajusta los límites y mira cómo ocurre la magia matemática.")

# 3. Entradas principales
col1, col2, col3 = st.columns([2, 1, 1])
with col1:
    func_input = st.text_input("Ingresa la función f(x):", value="x")
with col2:
...
try:
    ...
    with col5:
        n = st.slider("Número de rectángulos (n):", 1, 200, 38)
    ...
    ax.plot(x_vals, y_vals, 'b-', linewidth=2, label=f'$f(x) = {sp.latex(f_sym)}$')
    ax.bar(x_eval, y_eval, width=width, align=align, alpha=0.6, color='orange', edgecolor='black', linewidth=1.2)
    ax.set_xlim(a - margen, b + margen)
    ax.axhline(0, color='black', linewidth=1.2) 
    ax.axvline(a, color='red', linestyle='--', alpha=0.7) 
    ...
    st.error("Por favor, ingresa una función matemática válida.")